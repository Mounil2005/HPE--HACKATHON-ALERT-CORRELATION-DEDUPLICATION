...
export const createFacetsPanelStore = () =>
  createStore<FacetsPanelState>((set, state) => ({
    ...
    setFacets: (facets: FacetDto[]) => {
      // FacetsPanel calls this with the raw SWR value, which is undefined
      // until the first fetch resolves. Upstream never hits that because its
      // server-side prefetch always seeds the panel; here the prefetch can be
      // absent, so guard rather than crash the whole page on `.forEach`.
      if (!facets) {
        return;
      }
